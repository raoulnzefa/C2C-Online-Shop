    ...
    <div class="banner">
      <el-carousel :interval="5000" arrow="always" height="450px">
        <el-carousel-item v-for="item in banner" :key="item.id">
          ...
          <img v-if="item.picUr2" class="img2 a" :src="item.picUrl2" alt="">
          <img v-if="item.picUr3" class="img3 b" :src="item.picUrl3" alt="">
        </el-carousel-item>
      </el-carousel>
    </div>

    <div v-for="(item, i) in homeLists" :key="item.id">
      <div class="activity-panel" v-if="item.type===1">
        <el-row>
          <el-col class="content" :span="8" v-for='(o, index) in item.panelContents'>
...
export default {
  ...
  async created() {
    try {
      const res = await this.$http.get('http://localhost:8000/api/home',{
      })
      let data = res.data
      if (data.code === 200) {
